...
import { SEARCH_POKEMONS, SET_LOADING, GET_POKEMON } from "../types";

let clientId;
let clientSecret;

if (process.env.NODE_ENV !== "production") {
  clientId = process.env.REACT_APP_POKEMON_CLIENT_ID;
  clientSecret = process.env.REACT_APP_POKEMON_CLIENT_SECRET;
} else {
  clientId = process.env.POKEMON_CLIENT_ID;
  clientSecret = process.env.POKEMON_CLIENT_SECRET;
}
...
const PokemonState = (props) => {
  ...
  const searchPokemon = (search) => {
    setLoading();
    let results = [];
    if (search.length === 0) results = getAllData().data;
    else {
      results = getAllData().data.filter((pokemon) => {
        return pokemon.login.toLowerCase().includes(search.toLowerCase());
      });
    }
